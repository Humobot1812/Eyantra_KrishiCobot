...
# Functions:
#			        [ bad_fruit_detection, get_accurate_depth, pixel_to_3d, transform_optical_to_base_frame, create_transform_stamped, detect_aruco_markers, process_aruco_markers ]
...
class FruitsTF(Node):
    ...
    def process_aruco_markers(self, image, depth_image):
        """
        Detect ArUco markers, compute 3D positions like fruits, and publish TFs.
        """
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        ...
        for i, corner in enumerate(corners):
            ...
            if distance is None:
                self.get_logger().warn(f"No valid depth for ArUco {marker_id} at ({cX}, {cY})")
                continue
            ...
            # --- START FIX ---
            # Check if this marker is the fertilizer can
            # (Assuming the fertilizer is ArUco ID 3, based on your arm script)
            if marker_id == 3:
                frame_name = f"{self.team_id}_fertiliser_can"

            # Keep publishing aruco_6 for the arm script to use as a drop-off
            elif marker_id == 6:
                frame_name = f"{self.team_id}_aruco_6"

            # Ignore any other ArUco markers
            else:
                continue
            # --- END FIX ---
            ...
            self.get_logger().info(f"Published TF for ArUco {marker_id} at {base_pos}")
    ...
    def process_image(self):
        ...
        try:
            ...
            # Process ArUco markers (3D transform like fruits)
