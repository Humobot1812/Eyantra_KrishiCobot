...
"""
eBot Navigation for Task 2A with Shape Detection Integration
- Navigates through waypoints
- Pauses when shape is detected
...
print(f"Task 2A Waypoints: {waypoints}")

# Execution waypoints with intermediates
waypoints_exec = [
    [0.26, -1.95, 1.57],      # P1
    [-1.63, 1.27, 3.34],      # Intermediate
    [-1.70, 1.24, 3.14],      # Intermediate 
    [-1.48, -0.67, -1.45],    # P2
    [-1.53, -6.61, -1.57]     # P3
]
...
class EbotNavigator(Node):
    def __init__(self):
        ...
        self.get_logger().info('✓ Task 2A Navigator Started')
    ...
    def pause_callback(self, msg):
        ...
        if msg.data == "PAUSE":
            self.paused = True
            self.stop()
            self.get_logger().info("⏸️  Navigation PAUSED - Shape detected")
